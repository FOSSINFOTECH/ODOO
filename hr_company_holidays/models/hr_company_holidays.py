...
class hr_company_holidays(models.Model):
    ...
    month = fields.Char(string="Month", size=2, method=True, store=True, multi="get_value", compute='get_month_year')
    year = fields.Char(compute='get_month_year', size=4, string="Year", method=True, store=True, multi="get_value")
    ...
    def write(self, values):
        ...
        for part in user_obj.search([]):
            partners.append(part.partner_id.id)
        if values.get('description', False) or values.get('date', False):
            for val in self:
                calendar_vals = {
